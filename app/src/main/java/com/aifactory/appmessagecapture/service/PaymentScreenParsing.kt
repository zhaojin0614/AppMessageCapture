...
object PaymentScreenParsing {
     ...
     * 京东走「支付成功」页门槛；淘宝闪购（淘宝内频道 + 独立 App）走
     * 订单详情页门槛（[TaobaoShangouParsing.isOrderPage]，页面持久存在、
     * 无「支付成功」字样）。
     */
    fun isCapturePage(packageName: String, pageText: String): Boolean =
        when (packageName) {
            SupportedPaymentApps.TAOBAO_PACKAGE,
            SupportedPaymentApps.ELE_PACKAGE -> TaobaoShangouParsing.isOrderPage(packageName, pageText)
